...
public class RangeImpl implements Range {
    ...
    void receiveSplitData(Node node, Node newNode, int offset) {
        if (node == null || newNode == null || (fSplitNode == node)) return;
    ...
    void receiveDeletedText(CharacterDataImpl node, int offset, int count) {
        if ((node == null) || (fDeleteNode == node)) return;
        if (node == fStartContainer) {
    ...
    void receiveInsertedText(CharacterDataImpl node, int index, int len) {
        if ((node == null) || (fInsertNode == node)) return;
        if (node == fStartContainer) {
    ...
    public void insertedNodeFromDOM(Node node) {
        if ((node == null) || (fInsertNode == node) || fInsertedFromRange) return; // Offsets are adjusted in Range.insertNode
    ...
    void removeNode(Node node) {
        if ((node == null) || (fRemoveChild == node)) return;
    ...
    private Node getSelectedNode( Node container, int offset )
    {
        ...
        if ( (container.getNodeType() == Node.TEXT_NODE) || (offset<0) )
            return container;
