...
import net.sourceforge.htmlunit.xerces.impl.xs.XSConstraints;
...
class  XSDComplexTypeTraverser extends XSDAbstractParticleTraverser {
    ...
    private static XSWildcardDecl getErrorWildcard() {
        if (fErrorWildcard == null) {
            ...
            wildcard.fProcessContents = XSWildcard.PC_SKIP;
    ...
    throws ComplexTypeRecoverableError {
        ...
        // Set content type
        // -----------------------------------------------------------------------
        fContentType = XSComplexTypeDefinition.CONTENTTYPE_SIMPLE;
        fParticle = null;
        ...
        if ((type.getTypeCategory() == XSTypeDefinition.COMPLEX_TYPE)) {
            ...
            if (baseComplexType.getContentType() == XSComplexTypeDefinition.CONTENTTYPE_SIMPLE) {
            ...
            // base is a CT with mixed/emptiable content (only restriction is OK)
                    ...
                    baseComplexType.getContentType() == XSComplexTypeDefinition.CONTENTTYPE_MIXED &&
    ...
    throws ComplexTypeRecoverableError {
        ...
        if (fDerivedBy==XSConstants.DERIVATION_RESTRICTION) {
            ...
            if (fContentType == XSComplexTypeDefinition.CONTENTTYPE_MIXED &&
                    baseType.getContentType() != XSComplexTypeDefinition.CONTENTTYPE_MIXED) {
                fAttrChecker.returnAttrArray(complexContentAttrValues, schemaDoc);
                fAttrChecker.returnAttrArray(derivationTypeAttrValues, schemaDoc);
            ...
            else if (baseType.getContentType() == XSComplexTypeDefinition.CONTENTTYPE_EMPTY) {
            }
            else {
                //
                // Check if the contentType of the base is consistent with the new type
                // cos-ct-extends.1.4.3.2
                if (fContentType == XSComplexTypeDefinition.CONTENTTYPE_ELEMENT &&
                        baseType.getContentType() != XSComplexTypeDefinition.CONTENTTYPE_ELEMENT) {
                    fAttrChecker.returnAttrArray(complexContentAttrValues, schemaDoc);
                    fAttrChecker.returnAttrArray(derivationTypeAttrValues, schemaDoc);
                    throw new ComplexTypeRecoverableError("cos-ct-extends.1.4.3.2.2.1.a",
                            new Object[]{fName}, complexContent);
                }
                else if (fContentType == XSComplexTypeDefinition.CONTENTTYPE_MIXED &&
                        baseType.getContentType() != XSComplexTypeDefinition.CONTENTTYPE_MIXED) {
                    fAttrChecker.returnAttrArray(complexContentAttrValues, schemaDoc);
                    fAttrChecker.returnAttrArray(derivationTypeAttrValues, schemaDoc);
    ...
    throws ComplexTypeRecoverableError {
        ...
        // -----------------------------------------------------------------------
        if (fParticle == null)
            fContentType = XSComplexTypeDefinition.CONTENTTYPE_EMPTY;
        else if (isMixed)
            fContentType = XSComplexTypeDefinition.CONTENTTYPE_MIXED;
        else
            fContentType = XSComplexTypeDefinition.CONTENTTYPE_ELEMENT;
